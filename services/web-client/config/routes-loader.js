...
function resolveRoutes(relativePath) {
  return escape(path.resolve(paths.appSrc, relativePath))
}
...
module.exports = function routesLoader(source) {
  ...
  for (const route of routes) {
    ...
    const require = route.chunk && route.chunk === 'main' ?
      module => `Promise.resolve(require('${resolveRoutes(module)}').default)` :
      module => `new Promise(function (resolve, reject) {
        try {
          require.ensure(['${resolveRoutes(module)}'], function (require) {
            resolve(require('${resolveRoutes(module)}').default)
